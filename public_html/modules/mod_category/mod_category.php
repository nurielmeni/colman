...
if ($app->input->post->get('type', FALSE) === 'get_api') {
    ...
    if ($app->input->post->get('controller', FALSE) != "category") {
        return;
    }
...
//    $params['jobTitle'] = $_POST['jobTitle'];
    ...
    $params['catId'] = $app->input->post->get('catId', FALSE);
    $params['apply-job-comment'] = $_POST['apply-job-comment'];


    $obj->{$app->input->post->get('action', FALSE)}($params);
} else {
    $data = ModCategoryHelper::getAction($app->input->post->get('type', FALSE), $app->input->post->get('type', FALSE));
