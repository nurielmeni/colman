(function ($) {
  $.fn.jobList = function (options) {
    ...
    showHideDetails = function (id_pref, state, item) {
      if (state == 1) {
        $("#" + id_pref + "block" + item.id)
          .removeClass("joblist-short-details")
          .addClass("joblist-long-details");
      } else {
        $("#" + id_pref + "block" + item.id)
          .removeClass("joblist-long-details")
          .addClass("joblist-short-details");
      }

      var source =
        state == 1
          ? "plugins/system/nsoft/assets/js/jobList/html/single-job-details.html"
          : "plugins/system/nsoft/assets/js/jobList/html/single-job-short.html";
      jQuery.ajax({
        url: source,
        async: true,
        beforeSend: function () {
          if (state === 1) {
            $("#" + id_pref + "block" + item.id).before(
        ...
        success: function (data) {
          ...
          if (item.jobStatus == 3) {
            currentBlock
              .find(".joblist-apply")
              .parent()
              .append("<p class='joj-is-closed'>משרה סגורה</p>");
            ...
            currentBlock
              ...
              .parent()
              .prepend(
                "<p class='already-applied-job'>הוגשו קורות חיים למשרה זו</p>"
              );
            currentBlock.find(".joblist-apply").remove();
          } else {
            $.ajax({
              ...
              .always(function () {
                currentBlock
                  .find(".joblist-apply")
                  .attr("id", id_pref + "apply" + item.id)
                  .click(function () {
                    // settings.applyForJob(item.id)
                    if (item.customerSiteLink.length < 1 || cvEmailExist)
                ...
                if (item.customerSiteLink.length > 0) {
                  ...
                  $(
                    '<button class="popup_btn joblist-apply customer-site" onclick="window.open(\'' +
                      item.customerSiteLink +
                      "','_blank')\">הגש קו\"ח באתר המעסיק</button>"
                  ).insertBefore(currentBlock.find(".popup_btn.joblist-close"));
                }
              });
          }
          //console.log("---" + item);
          ...
          if (state == 1) {
            ...
            jQuery
              ...
              .done(function (res) {
                ...
                currentBlock
                  .find(".joblist-requirement")
                  .html(obj.Requirements);
                currentBlock.find(".joblist-skills").html(obj.Skills);
                var item = obj.rankList.find(function (elm) {
                  return elm["id"] == obj.Rank;
                });
                currentBlock
                  .find(".joblist-rank")
                  .html(typeof item === "undefined" ? "" : item.name);
                cvEmailExist =
                  obj.CvEmail == null || obj.CvEmail.length < 1 ? false : true;
                if (
                  !cvEmailExist &&
                  currentBlock.find(
                    "button.popup_btn.joblist-apply.customer-site"
                  ).length > 0
                ) {
                  currentBlock
                    .find("button.popup_btn.joblist-apply.local")
                    .remove();
                }
            ...
            currentBlock
              .find(".joblist-details")
              .attr("id", id_pref + "details" + item.id);
            currentBlock
              .find("#" + id_pref + "details" + item.id)
              .click(function () {
      ...
      jQuery
        ...
        .done(function (res) {
          ...
          if (res.fileList.length == undefined) {
            contentDiv
              .find("#apply-job-upload-file1-" + id_pref)
              .append(new Option(res.fileList.Name, res.fileList.FileId));
          } else {
            $.each(res.fileList, function (i, item) {
              contentDiv
                .find("#apply-job-upload-file1-" + id_pref)
                .append(new Option(item.Name, item.FileId));
            });
          }

          if (res.cvList.length == undefined) {
            contentDiv
              .find("#apply-job-upload-file2-" + id_pref)
              .append(
                new Option(
                  res.cvList.date + " " + res.cvList.name,
                  res.cvList.id
                )
              );
          } else {
            $.each(res.cvList, function (i, item) {
              contentDiv
                .find("#apply-job-upload-file2-" + id_pref)
                .append(new Option(item.date + " " + item.name, item.id));
            });
          }
          ...
          contentDiv.find("select.apply-job-upload-select").each(function () {
            ...
            if (
              $(this).attr("name") === "selectedFile[]" &&
              res.fileList.length > 0
            ) {
              placeholder = "קבצים מצורפים";
            }
            if (
              $(this).attr("name") === "selectedCv[]" &&
              res.cvList.length > 0
            ) {
              placeholder = "קורות חיים";
            }
            window.testSelAll2 = $(this).SumoSelect({
              selectAll: false,
    ...
    getApplyBlockData = function (curBlock, item, id_pref) {
      ...
      var options = {
        ...
        success: function (files) {
          //  console.log(files);
          curBlock
            .find("#apply-job-dropbox-fileName-" + jobId)
            .val(files[0].name);
          curBlock
            .find("#apply-job-dropbox-fileUrl-" + jobId)
            .val(files[0].link);
      ...
      jQuery.ajax({
        ...
        beforeSend: function () {
          $("#" + id_pref + "block" + item.id).before(
            '<progress class="pure-material-progress-linear apply-block"></progress>'
          );
        },
        success: function (data) {
          longDetailsHtml = $.parseHTML(data);
          ...
          curBlock
            .find("#jobId")
            .attr("id", "jobId_" + jobId)
            .val(jobId);
          curBlock
            .find("#jobCode")
            .attr("id", "jobCode_" + jobId)
            .val(item.jobCode);
          curBlock
            .find("#jobTitle")
            ...
            .val(item.name);
          curBlock
            .find("#apply-job-localFile")
            .attr("id", "apply-job-localFile-" + jobId);
          curBlock
            .find("#apply-job-upload-computer")
            .attr("id", "apply-job-upload-computer-" + jobId)
            .click(function () {
          ...
          curBlock
            ...
            .attr("id", "apply-job-localFile-" + jobId);
          curBlock
            .find("#apply-job-dropbox-fileUrl")
            .attr("id", "apply-job-dropbox-fileUrl-" + jobId);
          curBlock
            .find("#apply-job-dropbox-fileName")
            .attr("id", "apply-job-dropbox-fileName-" + jobId);
          curBlock
            .find("#apply-job-skydrive-fileUrl")
            .attr("id", "apply-job-skydrive-fileUrl-" + jobId);
          curBlock
            .find("#apply-job-skydrive-fileName")
            .attr("id", "apply-job-skydrive-fileName-" + jobId);
          ...
          curBlock
            .find("#apply-job-upload-dropbox")
            .attr("id", "apply-job-upload-dropbox-" + jobId);
          curBlock
            .find("#apply-job-upload-dropbox-" + jobId)
            .click(function () {
              Dropbox.choose(options);
            });

          curBlock
            .find("#apply-job-upload-skydrive")
            .attr("id", "apply-job-upload-skydrive-" + jobId);
          curBlock
            .find("#apply-job-upload-skydrive-" + jobId)
            .click(function () {
              WL.init({
          ...
          curBlock
            .find("#apply-job-submit")
            .attr("id", "apply-job-submit-" + jobId);
          curBlock
            .find("#apply-job-upload-form")
            .attr("id", "apply-job-upload-form-" + jobId);

          curBlock
            .find("#apply-job-upload-file1")
            .attr("id", "apply-job-upload-file1-" + jobId);
          curBlock
            .find("#apply-job-upload-file2")
            .attr("id", "apply-job-upload-file2-" + jobId);
          if (!isLogin) {
          ...
          curBlock.find("#apply-job-submit-" + jobId).click(function () {
            if (
              ($("#apply-job-upload-file2-" + jobId).val() != null &&
                $("#apply-job-upload-file2-" + jobId).val() !== "") ||
              (curBlock.find("#apply-job-upload-file-type").val() == "cv" &&
                (($("#apply-job-localFile-" + jobId).val() != null &&
                  $("#apply-job-localFile-" + jobId).val() !== "") ||
                  ($("#apply-job-dropbox-fileUrl-" + jobId).val() != null &&
                    $("#apply-job-dropbox-fileUrl-" + jobId).val() !== "") ||
                  ($("#apply-job-skydrive-fileUrl-" + jobId).val() != null &&
                    $("#apply-job-skydrive-fileUrl-" + jobId).val() !== "")))
            ) {
              var formData = new FormData(
                curBlock.find("#apply-job-upload-form-" + jobId)[0]
              );
              ...
              $.ajax({
                ...
                beforeSend: function () {
                  curBlock
                    .find("#apply-job-submit-" + jobId)
                    .before(
                      '<progress class="pure-material-progress-linear submit" style="margin-bottom: 20px;"></progress>'
                    );
                },
                async: true,
                success: function (result) {
                  ...
                  if (result == 0) {
                    alert(
                      "ERROR: Sorry but there was some problem. Please try again!"
                    );
                  } else {
                    //  debugger;
                    ...
                    $(
                      ".apply-job-upload-response.apply-job-upload-form"
                    ).show();
                    ...
                    if (
                      typeof resArray.fileList !== "undefined" &&
                      typeof resArray.fileList.type !== "undefined"
                    )
                      switch (resArray.fileList.type) {
                        case "cv":
                          console.log("cv: " + resArray.fileList.count);
                          $(".board-modul-box.cv .board-modul-count").text(
                            resArray.fileList.count
                          );
                          break;
                        case "file":
                          console.log("file: " + resArray.fileList.count);
                          $(".board-modul-box.files .board-modul-count").text(
                            resArray.fileList.count
                          );
                          break;
                        default:
                ...
                curBlock
                  .find("#apply-job-submit-" + jobId)
                  .parent()
                  .find("progress.submit")
                  .remove();
              });
            } else {
              alert(
                "קובץ קורות חיים לא נבחר:\nעליך לבחור קובץ קורות חיים מהקבצים הקיימים במערכת או להעלות קובץ קורות חיים חדש."
              );
            }
          });

          curBlock.find('.apply-job')[0].scrollIntoView();
        },
      });
      ...
      jQuery(document).on(
        "change",
        "#apply-job-upload-file-type",
        function (e) {
          //console.log("Meni: Upload File Type: " + $(this).val());
          switch (jQuery(this).val()) {
            case "cv":
              options.extensions = [".pdf", ".doc", ".docx", ".rtf"];
              jQuery("#apply-job-localFile-" + jobId).attr(
                "accept",
                "application/msword,application/vnd.openxmlformats-officedocument.wordprocessingml.document,application/pdf,application/rtf"
              );
              break;
            case "file":
              options.extensions = [
              ...
              jQuery("#apply-job-localFile-" + jobId).attr(
                "accept",
                "image/*,application/msword,application/vnd.openxmlformats-officedocument.wordprocessingml.document,application/pdf,application/rtf"
              );
              break;
          }
        }
    ...
    return $.each(settings.objList.jobs, function (i, item) {
      if (
        settings.objList.jobs.length <= settings.countPerPage ||
        i < settings.objList.jobs.length - 1
      ) {
        var container = $(document.createElement("li"))
          .addClass("popup_tmb joblist-short-details")
          .attr("id", settings.id_pref + "block" + item.id);
        ...
        jQuery.ajax({
          ...
          success: function (data) {
            longDetailsHtml = $.parseHTML(data);
            currentBlock = listContainer.find(
              "#" + settings.id_pref + "block" + item.id
            );
            currentBlock.html(longDetailsHtml);
            currentBlock.find(".joblist-jobName").text(item.name);
            currentBlock.find(".joblist-jobId").text(item.jobCode);
            ...
            currentBlock
              .find(".joblist-details")
              .attr("id", settings.id_pref + "details" + item.id);
            currentBlock
              .find("#" + settings.id_pref + "details" + item.id)
              .click(function () {
